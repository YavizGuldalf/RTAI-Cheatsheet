...
\textbf{UT-FGSM:} \mhl{$x' = x \textcolor{darkgreen}{+} \eta$, $\eta = \epsilon \cdot sign(\nabla_x loss_{\textcolor{darkgreen}{s}} (x))$}
...
\textbf{PGD:} Repeat UT-FGSM with \(\epsilon_{\mathrm{step}}\) and proj. to \(x \pm \epsilon\).
...
\textcolor{blue}{KGW(Red-Green)}: $\gamma|V|$ green tokens, rest red. Color chosen based on secret key derived from context (h).
Add $\delta$ to green logits during generation. To detect: count num green tokens S in L-len input, $p_{value} = 1-CDF_{binomial(L, \gamma)}(S)$
\textcolor{blue}{ITS}: Sequence $\xi$ and vocab perm. $\pi$ based on fixed secret. $x_t = \pi^{-1}(min\{\pi_i|\sum_{k \le i}P(\pi(k)) > \xi_x\}$. To detect $S=\sum_k|\xi_k-\frac{\pi(x_k)-1}{|V|-1}|$, then sample random secret T times. 
$p_{value} = \frac{1}{T+1}(1 + \sum_{t\in T}\boldsymbol{I}\{S(x,\xi,\pi) > S(x, \xi^{(t)},\pi^{(t)})\}$. \textcolor{blue}{SynthID}: Generate $m$ funcs $g_i: V \rightarrow \{0, 1\}$, each bit random, but PRF seed derived from context + secret. Then sample $2^m$ tokens from next logits and run a tournament. To detect $S = \sum_t\sum_{i\le m} g_i(x_t)$, $p_{value}=1-CDF_{Binomial(mL, 0.5)}(S)$
\textcolor{red}{Distortion free}(unchanged logit distribution): ITS \& SynthID. \textcolor{red}{Deterministic}: ITS, KGW. 
...
Benchmark is \textbf{standardized test or eval framework}. Consists of dataset (q\&a), scoring method (accuracy, pass\@k), standardized setup (prompt design).

\textbf{Steps to design}: Set the task, choose bench. paradigm, choose data source, create scoring mech., run model, interpret

\textbf{Benchmarking paradigms}: \textit{Closed-form}: MMLU, GSM8k | \textit{Free-form}: SWE-Bench, TruthfulQA | \textit{Simulation-based}: GameArena, LLM Pokemon | \textit{Preference}: LMArena, WildBench | \textit{Dynamic} (updated continuously): LiveBench, LiveCodeBench.

\textbf{Contamination types}: Data (benchmark samples in data, most common interpretation, improves ~5-10\%), Task (samples from popular benchmark overrepresented, hard to detect)

\textbf{Data contamination}: Sample $x$ contaminated if $y$ in $D_{\mathrm{train}}$ with $f(x,y)=1$ for some $f$ (often n-gram overlap - ie len of longest common sequence of words)

\textbf{Detection of contamination}:
\\Oracle Access (has $D_\mathrm{train}$): $\mathrm{ngram} (x,y) > 14$ 
\\White-Box Access (has weights): simplest is perplexity $\sum \log \mathbb P(\mathrm{next} \mid \mathrm{seq})$, most cited is avg log-ll of k\% least likely tokens threshold (perf close to random, unreliable).
Black-box Access (has outputs): rephrase sample, ask model to pick correct one (estimates \% of overlap $D_{\mathrm{bench}} \cap D_{\mathrm{train}}$). po = c*1 + (1-c)*pe

\textbf{Performance-based data contam}. when model performs better on bench than on a reference bench rel. to other models. Sample-level methods can be used: $Score(M, D_{contam}) - Score(M, D - D_{contam})$ (both contam. and non contam. data need to be big for stat. sign.)

\textbf{Evading detection}: train on rephrased. Detection is hard anyway. Mitigating contamination: private/dynamic benches.

\textbf{Scoring mech}: Accuracy, TPR\@x\%FPR, unit test, rubric (LLM-as-judge/human). Metrics are imperfect and can be cheated. Model answers are hard to parse/verify automatically (unexpected formatting in output). 

\textbf{Rating}: Bradley-Terry Model $P(M_1>M_2) = \frac{1}{1+exp(\tfrac{R_2-R1}{400})}$. Need to incorporate bias such as length, position, authority. $R_m = R_{base} + R_{len}\cdot len(x) + R_{auth} C(x)$. Fit Bradley-Terry with MLE: $R=\argmax \sum r\log P(M_1 > M_2) + (1-r) \log P(M_2 > M_1)$

\textbf{Dishonesty in real-world}: Benchmark omission, creative reporting, artificial increase.
